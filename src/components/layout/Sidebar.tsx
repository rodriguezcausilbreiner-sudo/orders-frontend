...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[220px] bg-white border-r border-slate-200 flex flex-col z-40">
      {/* Logo */}
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${
              isActive(item.href)
                ? 'bg-blue-50 text-blue-800 border-l-[3px] border-blue-800 pl-[9px]'
                : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
            }`}
          >
            <span className={isActive(item.href) ? 'text-blue-800' : 'text-slate-400'}>
              {item.icon}
            </span>
            {item.label}
          </Link>
      ...
      <div className="px-3 py-4 border-t border-slate-200">
        <div className="flex items-center gap-3 px-3 py-2 rounded-md bg-slate-50">
