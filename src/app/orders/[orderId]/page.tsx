...
export default function OrderDetailPage() {
  ...
  return (
    <AppShell>
      {/* Back + Header */}
      <div className="mb-6">
        <Link href="/orders" className="flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-3">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-slate-700">Customer Info</p>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-slate-700">Order Info</p>
          </div>
          <div className="space-y-2">
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Order Number</p>
              <p className="text-sm font-medium text-slate-800">#{order.orderNumber || order.id}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Order Date</p>
              <p className="text-sm text-slate-700">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-slate-700">Payment & Billing</p>
          </div>
          <div className="space-y-2">
            <div>
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Items</p>
              <p className="text-sm text-slate-700">{items.length} items ordered</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl mb-6">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-500">{items.length} Items Ordered</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <p className="text-sm font-semibold text-slate-900 mb-4">Order Timeline</p>
          <div className="space-y-4">
            {[
              { label: 'Order Placed', desc: new Date(order.orderDate).toLocaleString(), done: true },
              { label: 'Payment Confirmed', desc: 'Payment verified', done: true },
              { label: 'Sent to Fulfillment', desc: 'Pending allocation', done: false },
            ].map((step) => (
              <div key={step.label} className="flex items-start gap-3">
                <div className={`w-2.5 h-2.5 rounded-full mt-1 flex-shrink-0 ${step.done ? 'bg-blue-800' : 'bg-slate-200'}`} />
                <div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">Tax (Est. 8%)</span>
          ...
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-xs text-amber-700">
              This order is currently in <strong>{order.status ?? 'Processing'}</strong>.
              Changes to the order might cause fulfillment delays.
            </p>
          </div>
        </div>
      </div>
